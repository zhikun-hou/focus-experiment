...
expName = 'tests'  # from the Builder filename that created this script
datapath = sys.argv[1]
expName = "持续注意"
...
filename = os.path.abspath('../Debug')+ datapath[0:] + "/认知范式" +u'\\%s_%s' % (expName, expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='C:\\Users\\26633\\Desktop\\test_lastrun.py',
...
trials = data.TrialHandler(nReps=440, method='random', 
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [text, text_2, mouse, text_3]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *mouse* updates
        if mouse.status == NOT_STARTED and t >= 0.4-frameTolerance:
            # keep track of start time/frame for later
        ...
        if mouse.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    mouse.time.append(t-mouse.tStart)
        
        # *text_3* updates
        if text_3.status == NOT_STARTED and tThisFlip >= 1.4-frameTolerance:
            # keep track of start time/frame for later
        ...
        if text_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_3.tStartRefresh + 0.6-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials.addData('shownum',text_2.text)
    # store data for trials (TrialHandler)
    if len(mouse.leftButton): trials.addData('mouse.leftButton', mouse.leftButton[0])
    if len(mouse.time): trials.addData('mouse.reacttime', mouse.time[0])
